...
def render_scheduler_config():
    with st.form("config_form"):
        ...
        with col1:
            interval_tokens = st.number_input(
                "TokenStats обновление (сек)", 
                value=30, min_value=10, max_value=300
            )
        with col2:
            interval_exchanges = st.number_input(
                "ExchangesStats обновление (часы)", 
                value=1, min_value=1, max_value=24
            )
        with col3:
            interval_details = st.number_input(
                "Детальная информация (часы)", 
                value=24, min_value=1, max_value=48
            )

        submitted = st.form_submit_button("🔁 Запустить/Перезапустить")

        if submitted:
            restart_scheduler(interval_tokens, interval_exchanges, interval_details)


def restart_scheduler(interval_tokens, interval_exchanges, interval_details):
    st.session_state.scheduler.stop()
    st.session_state.scheduler = TaskScheduler(st.session_state.logs)
    st.session_state.scheduler.start(interval_tokens, interval_exchanges, interval_details)
    
    st.success(
        f"Планировщик запущен! TokenStats: {interval_tokens}с, "
        f"Exchanges: {interval_exchanges}ч, Details: {interval_details}ч"
    )
...
def render_manual_tasks():
    ...
    with col1:
        if st.button("📊 TokenStats", disabled=st.session_state.task_running):
            ...
            run_manual_task(
                manual_task_token_stats, 
                "Обновление TokenStats", 
                st.session_state.logs
            )
    ...
    with col3:
        if st.button("🔍 Детали токенов", disabled=st.session_state.task_running):
            ...
            run_manual_task(
                manual_task_token_details, 
                "Детальная информация токенов", 
                st.session_state.logs
            )
    ...
    with col4:
        if st.button("🏛️ Детали бирж", disabled=st.session_state.task_running):
            ...
            run_manual_task(
                manual_task_exchange_details, 
                "Детальная информация бирж", 
                st.session_state.logs
            )
